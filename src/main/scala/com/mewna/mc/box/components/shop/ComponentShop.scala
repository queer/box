...
class ComponentShop extends BoxedComponent {
  ...
  val shopitemMap: mutable.Map[String, ShopItem] =
    mutable.Map[String, ShopItem]()

  override def loadConfig(plugin: BoxPlugin): Unit = {
    // Load shop items
    plugin.getConfig
      .getConfigurationSection("shop.prices")
      .getKeys(false)
      .forEach(e => {
        ...
        try stack = Option(ItemUtil.get(mapKey))
        catch {
          case _: Exception =>
            try stack = Option(ItemUtil.get(e))
            catch {
              case e2: Exception =>
                plugin.getLogger.warning(
                  "Couldn't load shopitem at key '" + e + "' with name '" + name + "'!"
                )
                e2.printStackTrace()
                return
            }
        ...
        shopitemMap
          .put(mapKey, ShopItem(e, name, stack.get, buyPrice, sellPrice))
      })
  ...
  override def init(plugin: BoxPlugin): Boolean = {
    ...
    BoxedPlugin
      .locateComponent(classOf[SignRegistry])
