...
/**
  ...
  * @since 7/9/19.
  */
class ShopGui(override val plugin: Box,
              val shop: ComponentShop,
              val itemName: String)
  ...
  override protected def addButtons(): Unit = {
    try {
      ...
      case e: Exception =>
        ...
        addButton(
          ...
          getWithName(
            Material.BARRIER,
            ChatColor.RED + "Error loading shop! Please report this!"
          )
        )
        e.printStackTrace()
        return
    ...
    if (shopItem.isEmpty) {
      addButton(
        ...
        getWithName(
          Material.BARRIER,
          ChatColor.RED + "Item not found! Please report this!"
        )
      )
    } else {
      ...
      // Add buy stacks
      for (i <- 0 until 7) {
        val stack = getWithName(
          shopStack.getType,
          (ChatColor.WHITE + "Buy - $" + ChatColor.GREEN + "%.2f")
      ...
      for (i <- 0 until 7) {
        val stack = getWithName(
          shopStack.getType,
          (ChatColor.WHITE + "Sell - $" + ChatColor.RED + "%.2f")
  ...
  override protected def handleButtonClick(player: Player,
                                           button: GuiButton): Unit = {
    if (button.id < 7) {
        ...
        economyResponse.`type` match {
          case ResponseType.NOT_IMPLEMENTED | ResponseType.FAILURE =>
            ...
            MessageUtil.sendMessage(
              player,
              ChatColor.RED + "Couldn't withdraw money!"
            )
            return
      ...
      if (count < size) {
        ...
        response.`type` match {
          case ResponseType.NOT_IMPLEMENTED | ResponseType.FAILURE =>
            ...
            MessageUtil.sendMessage(
              player,
              ChatColor.RED + "Couldn't deposit money!"
            )
            return
          case ResponseType.SUCCESS =>
        }
  ...
  private def countMatInInv(p: Player, mat: Material) = {
    var count = 0
    for (stack <- p.getInventory.getContents
         if stack != null
