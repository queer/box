...
class ComponentMobStacking extends BoxedComponent {
  ...
  def count(entity: LivingEntity): Int = {
    ...
    if (customName != null && customName.startsWith(colour.toString)) {
      try entityCount = ChatColor.stripColor(customName).toInt
      catch {
        ...
        if (entity.isValid && isStackable(entity)) {
          ...
          if (removed > 0) {
            ...
            entity.setCustomName(
              f"$colour${Integer.toString(original + removed)}"
            )
          }
        }
      }
  ...
  private def check(a: Entity, b: Entity): Boolean = {
    ...
    || (a.asInstanceOf[Colorable].getColor == b
      .asInstanceOf[Colorable]
      .getColor))
  }
