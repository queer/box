...
/**
  ...
  */
object EnumUtil {
  def valueOf[T <: Enum[_]](enumClass: Class[T],
                            names: Seq[String]): Option[T] = {
    for (name <- names) {
      try {
  ...
  def getAllMatching[T <: Enum[_]](enumClass: Class[T],
                                   names: Seq[String]): util.Set[T] = {
  ...
  def getMaterial(names: Seq[String]): Option[Material] =
    valueOf(classOf[Material], names)

  def getStatistic(names: Seq[String]): Option[Statistic] =
    valueOf(classOf[Statistic], names)

  def getEntityType(names: Seq[String]): Option[EntityType] =
    valueOf(classOf[EntityType], names)
}
