...
object MessageUtil {
   ...
   */
  def sendMessagesPrefix(user: CommandSender, dummy: Int, messages: String*): Unit = {
   ...
   * @param message The message to send
   */
  def sendMessage(user: CommandSender, prefix: String, message: String): Unit = {
    sendMessage(user, prefix + ' ' + message)
  }
  ...
  def sendMessages(user: CommandSender, messages: Seq[String]): Unit = {
    user.sendMessage(messages.map(m => ChatColor.translateAlternateColorCodes('&', m)).toArray[String])
  }
   ...
   * @param messages The messages to send
   */
  def sendPrefixedMessages(user: CommandSender, prefix: String, messages: Seq[String]): Unit = {
