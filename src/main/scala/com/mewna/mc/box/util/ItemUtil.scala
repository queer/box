...
object ItemUtil {
  ...
  def setup(): Unit = {
    if (!isSetup) {
      ...
      val data = Source
        .fromInputStream(getClass.getResourceAsStream("/items.json"))
        .getLines
        .mkString("")
      loadJSON(data)
    }
  ...
  def get(id: String): ItemStack = {
    ...
    if (!material.isItem) {
      throw new IllegalStateException(
        "Unable to spawn item with material " + material
      )
    }
  ...
  def nameList(item: ItemStack): util.List[String] = {
    ...
    itemAliases.forEach(
      (k, _) =>
        if (k.equalsIgnoreCase(primaryName)) {
          names.add(k)
      }
    )
    names
  ...
  private def lookup(item: ItemStack) = {
    ...
    if (MaterialUtil.isPotion(`type`) && item.getItemMeta
          .isInstanceOf[PotionMeta]) {
  ...
  class ItemData {
    ...
    override def equals(o: Any): Boolean = {
      ...
      (getMaterial eq that.getMaterial) && potionDataEquals(that) && entityEquals(
        that
      )
    }
    ...
    private def entityEquals(o: ItemUtil.ItemData) =
      if (entity == null && o.getEntity == null) { // neither have an entity
        true
