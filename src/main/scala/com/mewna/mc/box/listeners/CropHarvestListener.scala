...
class CropHarvestListener extends Listener {
  ...
  def getApplicableDrops(item: Material): Seq[ItemStack] = {
    ...
    item match {
      case Material.WHEAT =>
        Seq(new ItemStack(Material.WHEAT), new ItemStack(Material.WHEAT_SEEDS, r.nextInt(4)))
      ...
      case Material.BEETROOTS =>
        Seq(new ItemStack(Material.BEETROOT),new ItemStack(Material.BEETROOT_SEEDS, r.nextInt(3)))
      case _ => Seq()
    }
  }
}
