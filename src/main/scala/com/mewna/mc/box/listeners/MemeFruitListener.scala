...
class MemeFruitListener extends Listener {
          ...
          for (i <- 0 until distance.asInstanceOf[Int]) {
            ...
            if (!passable && i > 0) {
              ...
              MessageUtil.sendMessage(
                event.getPlayer,
                f"Cannot meme through ${block.getType.name().toLowerCase()}."
              )
              return
            }
            lastLocation = loc
  ...
  private def lerp(start: org.bukkit.util.Vector,
                   end: org.bukkit.util.Vector,
                   percent: Double): org.bukkit.util.Vector = {
