...
object BoxedPlugin {
  ...
  def locateComponent[T](cls: Class[T],
                         ctx: Map[Class[_ <: Any], Any] = Map()): Option[T] =
    instance.locateComponent(cls, ctx)

  def locateCommandComponent[T](
    cls: Class[T],
    ctx: Map[Class[_ <: Any], Any] = Map()
  ): Option[T] = instance.locateCommandComponent(cls, ctx)
}
...
private class BoxedPlugin {
  ...
  def scanAndInitFromPlugin[T <: BoxPlugin](p: T): Unit = {
    ...
    if (boxed != null) {
      ...
      throw new IllegalArgumentException(
        f"Passed plugin of type ${cls.getName}, but it's not @Boxed!"
      )
    }
  }
  ...
  private def loadComponents(): Unit = {
    ...
    val componentClasses =
      graph.getClassesWithAnnotation(classOf[Component].getName)
    componentClasses.getNames.asScala
  ...
  private def registerListeners(): Unit = {
    val logger = plugin.get.getLogger
    val listenerClasses =
      graph.getClassesImplementing(classOf[Listener].getName)
    listenerClasses.getNames.asScala
      .map(n => Class.forName(n))
      .foreach(c => {
  ...
  private def registerCommands(): Unit = {
    val logger = plugin.get.getLogger
    val commandClasses =
      graph.getClassesWithAnnotation(classOf[Command].getName)
    commandClasses.getNames.asScala
      .map(n => Class.forName(n))
      .foreach(c => {
        commands.add(c)
        val annotation = c.getDeclaredAnnotation(classOf[Command])
        val wrapper = new BoxedCommand(
          c,
          annotation.name(),
          annotation.desc(),
          annotation.usage(),
          annotation.aliases(),
          annotation.permissionNode(),
  ...
  def injectComponents[T](obj: T,
                          ctx: Map[Class[_ <: Any], Any] = Map()): Unit = {
    obj.getClass.getDeclaredFields
      .filter(f => f.isAnnotationPresent(classOf[Auto]))
      .foreach(f => {
        ...
        if (ctxMatch.isDefined) {
          ...
          if (component.isDefined) {
            ...
            throw new IllegalArgumentException(
              f"Couldn't find component matching type ${t.getName}"
            )
          }
        }
      })
  ...
  def injectConfig[P <: BoxPlugin, T](plugin: P, obj: T): Unit = {
    ...
    obj.getClass.getDeclaredFields
      .filter(f => f.isAnnotationPresent(classOf[Config]))
      .foreach(f => {
        f.setAccessible(true)
  ...
  private def locateNormalComponent[T](
    cls: Class[T],
    ctx: Map[Class[_ <: Any], Any] = Map()
  ): Option[T] = {
    // Scan singletons
    ...
    if (singleGet.isDefined) {
      ...
      val singleClassMatch =
        singletons.keySet.find(c => cls.isAssignableFrom(c))
      if (singleClassMatch.isDefined) {
        ...
        if (option.isDefined) {
          val instance =
            option.get.getConstructor().newInstance().asInstanceOf[T]
  ...
  private def locateCommandComponent[T](
    cls: Class[T],
    ctx: Map[Class[_ <: Any], Any] = Map()
  ): Option[T] = {
