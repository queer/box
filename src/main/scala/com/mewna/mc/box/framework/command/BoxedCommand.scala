...
class BoxedCommand(
    ...
    src.getDeclaredMethods.filter(e => e.isAnnotationPresent(classOf[Subcommand])).foreach(m => {
      ...
      names.foreach(n => {
        if(subcommands.get(n).isDefined) {
          throw new IllegalStateException(f"Attempted to register subcommand '$n' for command class ${src.getName}, "
  ...
  override def execute(commandSender: CommandSender, cmdStr: String, args: Array[String]): Boolean = {
  ...
  private def executeCommand(sender: CommandSender, cmdStr: String, args: Array[String]): Boolean = {
    ...
    if(component.isDefined) {
      ...
      if(args.isEmpty) {
        ...
        if(maybeSubcommandMethod.isDefined) {
          maybeSubcommandMethod.get.invoke(instance, subcommandName, subcommandArgs)
        } else {
      ...
      throw new IllegalStateException(f"Asked to execute command of type ${src.getName}, " +
